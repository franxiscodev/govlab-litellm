# BASE DE CONOCIMIENTO AVANZADA: GUARDRAILS, CACHÉ SEMÁNTICA Y AUDITORÉA (2026)

---

## 6. GUARDRAILS, ANONIMIZACIÓN DE PII Y SEGURIDADN EN PROMPTS
...
LiteLLM permite forzar un System Prompt corporativo a nivel de pasarela para garantizar que ningun modelo responda a peticiones fuera de la política de uso corporativo.
...
## 7. CACHÉ SEMÁNTICA CON REDIS (REDUCCIÎN RADICAL DE COSTES)

### 7.1. Caché Exacta vs. Caché Semántica
- Caché Exacta: Retorna la respuesta guardada en Redis solo si el prompt es 100% idéntico caracter por caracter.
- Caché Semántica: Utiliza embeddings para determinar si la pregunta del usuario es similar en un 85%+ a una pregunta realizada previamente, respondiendo de inmediato desde Redis con tiempo de respuesta <10ms y costo $0.00.

377.2. Configuración de Caché en config.yaml:
```config.yaml
litellm_settings:
  cache: true
  cache_params:
...
## 8. CONSULTAS SQL PARA AUDITORÁ DE CFO & INFORMES DE GASTO

Mientras LiteLMM almacena todas las transacciones en PostgreSQL (LiteLLM_SpendTable y LiteLLM_VerificationToken).
...
8.2. Consulta 2: Top 5 modelos más consumidos y su costo promedio por petició>
```gsql
SELECT 
    model,
...
8.3. Consulta 3: AuditorÁ de Virtual Keys que alcanzaron su límite de presupuesto
```gsql
SELECT 
    token,
    key_name,
...
## 9. OBSERVABILIDAD, PROMETHEUS Y ALERTAS EN RLACK
...
## 10. INTEGRACIÎN EN APLICACIONES CLIENTE (SDKs, LANGCHAIN & LLAMAINDEX)
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=[{"role": "user", "content": "ÍComo optimizar la bateria en servidores?"}]
)

print(response.choices[0].message.content)
```

10.2. Conexivón desde LangChain
```app.py
...
10.3. Conexivón desde LlamaIndex
```python
